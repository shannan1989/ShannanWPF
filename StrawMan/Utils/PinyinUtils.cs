...
{
    internal class PinyinUtils
    {
        #region 一级汉字及对应拼音

        private static int[] firstLevelChCode = new int[]
        {
        ...
        public static string getPinyin(char ch)
        {
            ...
            else if (chr <= lastOfFirstLevelChCode)// 如果是在一级汉字中
            {
                // 将一级汉字分为12块,每块33个汉字.
                for (int aPos = 11; aPos >= 0; aPos--)
                {
                    ...
                    if (chr >= firstLevelChCode[aboutPos])
                    {
                        // Console.WriteLine("存在于第 " + aPos.ToString() + " 块,此块的第一个机内码是: " + firstLevelChCode[aPos * 33].ToString());
                        // 遍历块中的每个音节机内码,从最后的音节机内码开始扫描,
                        // 如果音节内码小于机内码,则取此音节
                        for (int i = aboutPos + 32; i >= aboutPos; i--)
                        {
                            if (firstLevelChCode[i] <= chr)
                            {
                                // Console.WriteLine("找到第一个小于要查找机内码的机内码: " + firstLevelChCode[i].ToString());
                                return firstLevelChPinyin[i];
                            }
                        }
                        break;
                    }
        ...
        public static string getPinyinInitial(char ch)
        {
            var pinyin = getPinyin(ch);
        ...
        public static string getPinyinInitial(string str)
        {
            ...
            {
                sb.Append(getPinyinInitial(chs[i]));
            }
